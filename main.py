...
# Commenter can delete his/her comment
...
class Comment(db.Model):
    ...
    # Foreign key linking to BlogPost
...
def register():
    ...
    if form.validate_on_submit():
        #Virfivy if user exist
        existing_user = db.session.execute(db.select(User).where(User.email == form.email.data)).scalar()
        if existing_user:
            flash("Este usuario ya está registrado. Intente hacer log in.", "warning")
        ...
        db.session.commit()
        flash("Ususario registrado con exito.", "success")
    ...
    if form.validate_on_submit():
        ...
        if user:
            if check_password_hash(user.password, password):
                ...
                return redirect(url_for("get_all_posts"))  # o la ruta que prefieras
            else:
